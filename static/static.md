...
Let's turn to `gdb` and explore our program. `gdb` is the GNU Debugger and is incredibly powerful. Often, the fancy shmancy visual debuggers found in IDE's are actually just front ends which hide `gdb` under the hood.

In order to use `gdb` most effectively, the executable you are debugging must be built using the `-g` option to either the `C` or `C++` compilers. Specifying the debugging option:

* causes a tremendous amount of instrumentation code to be added to every higher level language function and
* is mutually exclusive of any added optimization (such as that provided by the `-O` compiler options)

Here is how the program above is built to enable debugging:

```text
gcc -g static02.s
```

`gdb` is launched by giving it a path to the executable to be debugged. As in:

```text
gdb ./a.out
```

In this, what may be your first experience with `gdb`, we will focus on a small number of commands:

* `b` sets a breakpoint at either a label within the code or at a line number.
* `p` is for `p`rinting the contents of registers, for example.
* `x` is for e`x`aminging the contents of memory.
* `n` executes the current instruction
...
Above, we confirm that the string `s2` has a typo. Since the string is in memory, the `x` command is required and since it is a string, the `s` switch is added.
...
If you need to specify command line arguments to the program to be run, you can do so after the `run` command. It feels odd, yes, to specify command line arguments without specifying the name of the program to run but this was done when we launched `gdb`.

`gdb` always prints **the next line of code** to be executed.
...
This time, the program has crashed. The crash is because `s1` is sitting in memory that has been tagged as being *read-only*. The attempt to write to the read-only data caused the Seg Fault, killing the program.

## A Note About Using `gdb` on a Mac

It is a major pain to use `gdb` on the Macintosh. Why? Apple. That's why.

Instead, Apple's `lldb` works essentially the same way and is the debugger which goes along with `clang` which masquerades as `gcc`, `g++`, `clang`, `clang++`, `as`, `cc`, `c++` and `cpp` and more. Oh yes, Apple has no issue at all with lying to you. Why? Apple. That's why.
